...
def settle(positions_0, positions_1, residues, masses, tol=0.0001, dists=[1,1.632981]):
    """
    """
    pos_0 = positions_0[residues,:]
    oxy_0 = pos_0[:,0,:]
    ha_0 = pos_0[:,1,:]
    hb_0 = pos_0[:,2,:]
    
    pos_1 = positions_1[residues,:]
    oxy_1 = pos_1[:,0,:]
    ha_1 = pos_1[:,1,:]
    hb_1 = pos_1[:,2,:]

    mass = masses[residues]
    m_oxy = mass[0,0]
    m_ha = mass[0,1]
    m_hb = mass[0,2]
    ...
    xcom = (xp0*m_oxy + (xb0+xp1)*m_ha + (xc0+xp2)*m_hb) / (m_oxy + m_ha + m_hb)
    ...
    delx = 2.0*xb2d + (4.0*xb2d2 - hh2 + dists[1]**2)**0.5
    xb2d = xb2d - 0.5*delx
    ...
    xa3 = np.concatenate((xa3.reshape(-1,1), ya3.reshape(-1,1), za3.reshape(-1,1)), axis=1)
    xb3 = np.concatenate((xb3.reshape(-1,1), yb3.reshape(-1,1), zb3.reshape(-1,1)), axis=1)
    xc3 = np.concatenate((xc3.reshape(-1,1), yc3.reshape(-1,1), zc3.reshape(-1,1)), axis=1)
